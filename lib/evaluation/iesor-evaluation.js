...
function iesorevaluation(backbone, globalConfig, localConfig)
{
	...
	// self.simulationTimeMS = searchParams.simulationTimeMS || 3500;
	...
	var evalCleared = false;
	var measureCleared = false;

	self.eventCallbacks = function()
	{ 
		return {
			"evaluate:clearEvaluations" : function()
			{
				//gotta kill the evals -- coming from the top
				self.allEvaluations = {};
				evalCleared = true;
				measureCleared = true;
			},
			"evaluate:evaluateArtifacts" : function(population, finished){
				...
				// {
					...
					// var err = new Error();
					// self.log(err.stack);
					// finished("Evolution cleared in the middle of callback");
					// done();
					// return;
				// }
				...
				//this is a flow control call -- send in our global search params -- that can be altered at any time!
				self.backEmit.qCall("iesor:simulateArtifacts", pending, searchParams)
			...
			,"evaluate:measureObjectives" : function(population, popEvaluations, finished)
			{
				// if(measureCleared)
				// {
				// 	measureCleared = false;
				// 	evalCleared = false;
				// 	finished();
				// 	// finished("Evolution cleared in the middle of measure callback");
				// 	return;
				// }
				...
				self.backEmit.qConcurrent(measureCalls)
					.then(function(allResults)
					{
						...
						for(var wid in widToDistances)
						{
							...
							var nearest = nMeasured.nearestNeighbors;
						 	for(var nWID in nearest)
						 	{
						 		...
						 		if(!neighborEval)
						 		{
						 			...
						 			//neighbor eval might be deleted in the middle of this callback
						 			if(!neighborEval)
						 				neighborGenomeDistance = -1;
						 			else
						 				neighborGenomeDistance = neighborEval.genomeDistance;
						 		}
						 		else if(!widToDistances[nWID])
						 		{
						 			var aEval = self.allEvaluations[nWID];
						 			if(aEval)
						 				neighborGenomeDistance = aEval.genomeDistance;
						 			else
						 				neighborGenomeDistance = -1;
						 		}
						 		...
						 		//issue with canceling this evolution during eval
						 		if(!neighborEval)
						 			neighborEval = {};
							...
							// self.log("Local genom div: ", nMeasured.novelty);
							// self.log("Local genom div: ", localGenomeDiversity/nCount);

							//here we modify novelty by how genetically diverse you are in the same neighborhood
						 	gObjectives[gObjectives.length-1] = nMeasured.novelty*(1 + localGenomeDiversity/nCount);

							//genome distance is the last objective
						 	// gObjectives[gObjectives.length-2] = self.nsgaGlobalGenomeDiversity ? genomeDist : localGenomeDiversity/nCount; //local genome diversity? or not;
	...
	self.requiredEvents = function() {
		return [
		//need to be able to reach cpp iesor sim
			"iesor:simulateArtifacts",
			//need to get genome distances from each other -- inside neat
			"encoding:NEATGenotype-measureGenomeDistances",
			//neat to measure novelty amongs the pedestrian artifacts 
